...
let Brie = Cisyxy[0]              //jsBridge
let amt = Cisyxy[1]     //amount
let ren = Cisyxy[2]      //currency
let OpWin = Cisyxy[3]      //openWindow
...
internal class FetsCzauViewController: UIViewController,WKNavigationDelegate, WKUIDelegate, WKScriptMessageHandler {

    var djnaue: Unasic?
    var cmoanje: WKWebView?
    ...
    override func viewDidLoad() {
        ...
        if djnaue!.mnikao != nil {
            view.backgroundColor = UIColor.init(hexString: djnaue!.mnikao!)
        }
        
        AppsFlyerLib.shared().appsFlyerDevKey = djnaue!.lznhe!
        AppsFlyerLib.shared().appleAppID = djnaue!.zasue!
...
//        
        ...
        Cisyxy = djnaue!.vyuxh!.components(separatedBy: ",")
//        HuntOrderKrajs = [aTc,diaChon, diyicicho, hdrawo, geicho, chozh, diyichCha, deop]
        let usrScp = WKUserScript(source: djnaue!.xydye!, injectionTime: .atDocumentEnd, forMainFrameOnly: true)
        ...
        cmoanje = WKWebView(frame: .zero, configuration: cofg)
        cmoanje!.allowsBackForwardNavigationGestures = true
        cmoanje?.uiDelegate = self
        cmoanje?.navigationDelegate = self
        view.addSubview(cmoanje!)
        
        kaoieus = djnaue!.qwaiux!
        if kaoieus?.contains("://") == true {
            cmoanje?.load(URLRequest(url:URL(string: kaoieus!)!))
        } else {
            cmoanje?.load(URLRequest(url:URL(string: Uznxiiso(kaoieus!)!)!))
        }
    ...
    override func viewDidLayoutSubviews() {
        ...
        if let ws = UIApplication.shared.connectedScenes.first as? UIWindowScene,
           let statusBarManager = ws.statusBarManager {
            ...
            let statusBarHeight = djnaue!.kosoii!.contains("V") ? statusBarManager.statusBarFrame.height : 0
            let bottomHeight = djnaue!.kosoii!.contains("I") ? view.safeAreaInsets.bottom : 0
            cmoanje?.frame = CGRectMake(0, statusBarHeight, view.bounds.width, view.bounds.height - statusBarHeight - bottomHeight)
        }
    }
